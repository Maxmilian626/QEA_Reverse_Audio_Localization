...
[desired_sound, fs] = audioread('test.wav');
f = 'test_loop_wo_hamming.wav';
loop_step = 5;

interval_num = ceil(360/loop_step); %break into this many chunks, ceil to be conservative
...
%     amp_shift = amp_interval_slope * closest_angle_diff;
    ...
    next_closest_time_shift = - round(time_interval_slope * (data_step_size - closest_angle_diff));

    %get indices of max amp for the L and R microphones
    closest_soundL = closest_impulse(:,1);
    closest_soundR = closest_impulse(:,2);
    next_closest_soundL = next_closest_impulse(:,1);
    next_closest_soundR = next_closest_impulse(:,2);
    [closest_max_soundL, closest_indexL] = max(closest_soundL);
    [closest_max_soundR, closest_indexR] = max(closest_soundR);
    [next_closest_max_soundL, next_closest_indexL] = max(closest_soundL);
    [next_closest_max_soundR, next_closest_indexR] = max(closest_soundR);
    ...
    if ear == 0 %if R, - slope should make R happen less ahead of L
        ...
        next_closest_time_before_indexR = time_before + next_closest_time_shift
        next_closest_time_after_indexR = time_after + next_closest_time_shift
    ...
    next_closest_time_before_indexL = time_before;
    next_closest_time_after_indexL = time_after;
    ...
    if next_closest_indexL > next_closest_indexR
        next_closest_index = next_closest_indexR;
    else
        next_closest_index = next_closest_indexL;
    end
    ...
    next_closest_sound_time_adjustedL = next_closest_soundL(next_closest_index - next_closest_time_before_indexL:next_closest_index + next_closest_time_after_indexL);
    next_closest_sound_time_adjustedR = next_closest_soundR(next_closest_index - next_closest_time_before_indexR:next_closest_index + next_closest_time_after_indexR);

    sound_amp_adjustedR = closest_sound_time_adjustedR * closest_proportion + next_closest_sound_time_adjustedR * next_closest_proportion;
    sound_amp_adjustedL = closest_sound_time_adjustedL * closest_proportion + next_closest_sound_time_adjustedL * next_closest_proportion;
